...
const groupCreatorFormContainer = document.getElementById(
  "group-creator-form-container"
);
...
socket.on("updatedUserList", (users, user) => {
  ...
  if (user === receiverInfo.innerText) {
    ...
    outputMessage({
      userName: "Server",
      text: `\\i\\B\\blue${user}\\blue\\B\\i \\redhas left.\\red`,
...
chatForm.addEventListener("submit", (e) => {
  ...
  if (mode != "group") {
    ...
    outputMessage({ userName: "You", text, time: moment().format("h:mm a") });
    chatMessages.scrollTop = chatMessages.scrollHeight;
    socket.emit("groupChatText", text);
  }
...
groupCreatorForm.addEventListener("submit", (e) => {
  e.preventDefault();
  groupCreatorFormContainer.style.display = "none";
...
socket.on("broadcastMessageOnJoin", (message) => {
  outputMessage(message);
  chatMessages.scrollTop = chatMessages.scrollHeight;
});
...
socket.on("receiverDeselection", (user) => {
  for (let i = 0, len = userList.children.length; i < len; i++) {
    if (
      ...
      userList.children[i].id === user.id
    ) {
    ...
    li.addEventListener("click", (e) => {
      if (selectedReceiver.id !== user.id) {
        ...
        for (let i = 0, len = chatMessages.childNodes.length; i < len; i++) {
          if (
            chatMessages.childNodes[i].innerText.split(/\d/)[0].trim() !==
            user.username
          ) {
...
function outputNotification(text, toNotificationArea1 = 0) {
  ...
  function clickHandler(e) {
    ...
    if (activeUsers.length) {
        ...
        if (e.target.id === userList.childNodes[i].id) {
          ...
          if (toNotificationArea2) {
            ...
            notificationIcon.innerText =
              notificationArea.childElementCount == 0
                ? ""
      ...
      outputMessage({
        userName: "Server",
        text: `\\h3\\redNo active user!\\red\\h3`,
        time: moment().format("h:mm a"),
      });
    }
    if (!notificationIcon.innerText) {
...
document.getElementById("create-group").addEventListener("click", (e) => {
  dropdownContent.style.display = "none";
  groupCreatorForm.elements.btn.innerText = "Create Group";
  groupCreatorFormContainer.style.display = "block";
});

document.getElementById("join-group").addEventListener("click", (e) => {
  dropdownContent.style.display = "none";
  groupCreatorForm.elements.btn.innerText = "Join Group";
  groupCreatorFormContainer.style.display = "block";
});
